...
if ($_POST['act_button'] == "선택수정") {
    ...
    for ($i=0; $i<count($_POST['chk']); $i++)
    {
        ...
        $branch_id = $_POST['branch_id'][$k];

        if (!$branch_id) {
            $msg .= $mb['mb_id'].' : 지회자료가 존재하지 않습니다.\\n';
        } else {
            // 지회자료 삭제
            sql_query("DELETE FROM `branch` WHERE branch_id = '{$branch_id}' AND reunion_id = '{$reunionID}'");
            // 지회회원자료 삭제
            sql_query("DELETE FROM `branch_member` WHERE branch_id = '{$branch_id}' ");
        }
    }
}
